...
export function setupSuccessfulPaymentMocks(
    ...
    (mockMessaging.subscribe as jest.Mock).mockImplementation((channel, event, callback) => {
        setTimeout(() => {
            callback({
                orderId,
...
export function setupNetworkErrorMocks() {
    MockedPaymentApi.mockImplementation((() => ({
        createVivaTransactionV3: jest.fn().mockRejectedValue(new Error('Network error')),
        cancelTransaction: jest.fn(),
    })) as any);
}
